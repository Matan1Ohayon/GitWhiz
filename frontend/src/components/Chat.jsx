...
export default function Chat({ repoName }) {
  ...
  async function handleSend() {
    ...
    setLoading(true)
    try {
      const data = await sendMessage(repoName, question)
  ...
  return (
    <div className="flex flex-col h-full bg-white border border-gray-200 dark:bg-[#0f1724] dark:border-white/10 rounded-2xl overflow-hidden">
      ...
      <div className="flex-1 flex flex-col gap-3 sm:gap-4 p-3 sm:p-4 overflow-y-auto min-h-0 max-h-[50vh] lg:max-h-[calc(100vh-300px)]">
        {messages.map((msg, i) => (
          <div key={i} className={`flex gap-1.5 sm:gap-2 items-end ${msg.role === 'user' ? 'flex-row-reverse' : ''}`}>
            ...
            <div className={`text-xs sm:text-sm leading-relaxed px-3 sm:px-4 py-2 sm:py-3 rounded-2xl max-w-[90%] sm:max-w-[85%] overflow-hidden break-words ${
              msg.role === 'user'
      ...
      <div className="flex gap-2 p-3 sm:p-4 border-t border-gray-200 dark:border-white/10 flex-shrink-0">
        ...
        <button
          onClick={handleSend}
          className="w-9 h-9 sm:w-10 sm:h-10 rounded-xl bg-gradient-to-r from-cyan-dark to-rose flex items-center justify-center text-white hover:scale-105 transition-all flex-shrink-0 text-sm"
        >
          ➤
        </button>
      </div>
